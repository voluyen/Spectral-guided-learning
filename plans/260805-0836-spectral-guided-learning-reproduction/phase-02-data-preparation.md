...
4. Segmentation (**reasoning step definition**): split CoT trajectories at logical boundaries using regex `r'([.?!\}\]])([\s\n]+)([A-Z])'` — cut before the capital letter, i.e. after sentence-ending punctuation (or a closing `}`/`]` ending inline LaTeX) plus whitespace. Deterministic, preserves semantic completeness. **No merge step** — every boundary found becomes a step, short steps are kept as-is (only zero-token pieces are dropped). Map each step to a token range from the character offsets of a **single** tokenizer pass (`return_offsets_mapping`), bisecting boundary character positions into token indices. The response is never tokenized piecewise — BPE is context-dependent, so concatenating separately-encoded steps yields non-canonical ids (a lone whitespace token per step start, ~4% token inflation).
