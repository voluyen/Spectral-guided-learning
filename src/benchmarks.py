...
def load_aime24() -> list[dict]:
    ...
    rows = load_dataset("math-ai/aime24", split="test")
...
def load_aime25() -> list[dict]:
    rows = load_dataset("math-ai/aime25", split="test")
...
def load_olympiadbench() -> list[dict]:
    ...
    rows = load_dataset("Hothan/OlympiadBench", "OE_TO_maths_en_COMP", split="train")
    records = []
    for index, row in enumerate(rows):
