#!/usr/bin/env bash
# Phase 5: masked SFT -- SPECTRAL, Qwen3-8B track (LoRA). No vanilla run: P-ALIGN's own
# "SFT (Long-CoT)" row is the reference baseline.
set -euo pipefail
...
DATA_PATH="${BASE_PATH}/data/qwen3-8b/train-spectral.jsonl"
OUTPUT_DIR="${BASE_PATH}/checkpoints/spectral-qwen3-8b"
EPOCHS=3
LR=5.0e-5
MIN_LR=1.0e-5
WARMUP_RATIO=0.1
