#!/usr/bin/env bash
# Phase 5: masked SFT -- VANILLA baseline, Qwen3-1.7B track (LoRA, DeepSpeed ZeRO-2, multi-GPU).
set -euo pipefail
...
DATA_PATH="${BASE_PATH}/data/qwen3-1.7b/train-vanilla.jsonl"
OUTPUT_DIR="${BASE_PATH}/checkpoints/vanilla-qwen3-1.7b"
EPOCHS=3
LR=5.0e-5
MIN_LR=1.0e-5
WARMUP_RATIO=0.1
...
echo "${CMD}"
${CMD} 2>&1 | tee "${BASE_PATH}/logs/sft-qwen3-1.7b.log"
