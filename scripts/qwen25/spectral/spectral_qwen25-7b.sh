#!/usr/bin/env bash
# Phase 5: masked SFT -- SPECTRAL, Qwen2.5-7B-Instruct track (LoRA). No vanilla run: P-ALIGN's
...
OUTPUT_DIR="${BASE_PATH}/checkpoints/spectral-qwen25-7b"
EPOCHS=3
LR=5.0e-5
MIN_LR=1.0e-5
WARMUP_RATIO=0.1
...
echo "${CMD}"
${CMD} 2>&1 | tee "${BASE_PATH}/logs/spectral-qwen25-7b.log"
